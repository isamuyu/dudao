...
import { buildFacilityRows, facilityName } from '../../checklib/checklib';
  ...
  async submit(user: AuthUser, dto: CreateInspectionDto) {
    return this.dataSource.transaction(async (em) => {
      ...
      if (!blocked) {
        ...
        const facilityRows = buildFacilityRows(point.subtypeId);
            ...
            issuesRepo.create({
              ...
              desc:
                row.level === 'M'
                  ...
                  : `${facilityName(row.facility)}为条件设置项，督导员现场确认触发条件（${row.condition ?? '详见标准'}）已满足，但未设置该设施。`,
              photos: [],
              status: 'open',
              history: [
